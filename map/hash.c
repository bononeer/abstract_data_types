...
static void hash_table_destroy(pair_t* table, size_t capacity, void (*value_destroy)(void* value));
...
static bool hash_table_resize(Map hash, size_t new_capacity) {
    ...
    size_t old_capacity = hash->capacity, old_size = hash->size, old_deleted = hash->deleted;
...
static size_t hash_search(Map hash, const char* key) {
    ...
    for ( ; hash->table[index].state != EMPTY ; index = (index+1) % hash->capacity) {
        if (hash->table[index].state == TAKEN && strcmp(hash->table[index].key, key) == 0) return index;
    }
