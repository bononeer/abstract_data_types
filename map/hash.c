...
bool map_iter_has_next(const MapIterator iter) {
    return iter->current_index < iter->hash->capacity;
}

bool map_iter_next(MapIterator iter) {
    if (!map_iter_has_next(iter)) return false;
...
const char* map_iter_get_current(const MapIterator iter) {
    return map_iter_has_next(iter) ? iter->hash->table[iter->current_index].key : NULL;
}
